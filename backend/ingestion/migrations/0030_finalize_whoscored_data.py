...
def classify_stored_own_goals(apps, schema_editor):
    ...
    payloads = (
        ProviderMatchPayload.objects.using(database)
        .exclude(payload_gzip=None)
        .only("provider_match_id", "payload_gzip")
...
def build_match_carries(ProviderMatchCarry, provider_match):
    ...
    for (previous_index, previous), (event_index, event) in zip(located, located[1:]):
        ...
        distance = distance_metres(start, end)
        if not 3.0 <= distance <= 60.0:
            continue
        if previous.event_type in ACQUISITION_TYPES and (
            elapsed_seconds == 0 or (elapsed_seconds == 1 and distance > 6.0)
        ):
            continue
        rows.append(
            ProviderMatchCarry(
                ...
                is_low_confidence=(
                    previous.event_type in ACQUISITION_TYPES and elapsed_seconds == 1
                ),
            )
        )
    return rows
...
def carry_signature(carry):
    return (
        carry.start_event_index,
        carry.end_event_index,
        carry.provider_team_id,
        carry.team_id,
        carry.provider_player_id,
        carry.player_id,
        carry.period,
        carry.minute,
        carry.second,
        ...
        carry.is_final_third_entry,
        carry.is_box_entry,
        carry.is_low_confidence,
    )
...
def rebuild_carries(apps, schema_editor):
    ...
    database = schema_editor.connection.alias
    matches = (
        ProviderMatch.objects.using(database)
        .prefetch_related("events")
        .order_by("pk")
    )
    for provider_match in matches.iterator(chunk_size=100):
        rows = build_match_carries(ProviderMatchCarry, provider_match)
        existing = list(
            ProviderMatchCarry.objects.using(database)
            .filter(provider_match_id=provider_match.id)
            .order_by("start_event_index")
        )
        if [carry_signature(carry) for carry in existing] == [
            carry_signature(carry) for carry in rows
        ]:
            continue
        with transaction.atomic(using=database):
            ProviderMatchCarry.objects.using(database).filter(
                provider_match_id=provider_match.id
            ).delete()
            ProviderMatchCarry.objects.using(database).bulk_create(rows, batch_size=1000)


def finalize_whoscored_data(apps, schema_editor):
    classify_stored_own_goals(apps, schema_editor)
    rebuild_carries(apps, schema_editor)


class Migration(migrations.Migration):
    atomic = False

    dependencies = [("ingestion", "0029_whoscored_final_schema")]

    operations = [
        migrations.RunPython(finalize_whoscored_data, migrations.RunPython.noop),
    ]
