...
def ingest_understat_slice(competition_season: CompetitionSeason, *, run: IngestionRun) -> None:
    ...
    try:
        with transaction.atomic():
            ...
            to_create: list[UnderstatPlayerSeasonSource] = []
            for row in players:
            ...
            if source_rows:
                UnderstatPlayerSeasonSource.objects.bulk_update(
                    source_rows,
                    ["canonical_player", "canonical_team"],
                    batch_size=1000,
                )
    except Exception as exc:  # noqa: BLE001
...
def ingest_sofascore_slice(competition_season: CompetitionSeason, *, run: IngestionRun) -> None:
    ...
    try:
        with transaction.atomic():
            ...
            to_create: list[SofascorePlayerSeasonSource] = []
            ...
            for src in source_rows:
                ...
                src.canonical_team = cteam
            if source_rows:
                SofascorePlayerSeasonSource.objects.bulk_update(
                    source_rows,
                    ["canonical_player", "canonical_team"],
                    batch_size=1000,
                )
...
def ingest_sofascore_team_slice(competition_season: CompetitionSeason, *, run: IngestionRun) -> None:
    ...
    try:
        with transaction.atomic():
            ...
            to_create: list[SofascoreTeamSeasonSource] = []
            for row in rows:
            ...
            source_rows = SofascoreTeamSeasonSource.objects.bulk_create(to_create, batch_size=1000)
            for src in source_rows:
                provider_team_id = src.provider_team_id
                cteam = None
                if provider_team_id:
                ...
                src.canonical_team = cteam
            if source_rows:
                SofascoreTeamSeasonSource.objects.bulk_update(
                    source_rows,
                    ["canonical_team"],
                    batch_size=1000,
                )
