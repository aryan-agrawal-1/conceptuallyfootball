...
from ingestion.services.ingest import ingest_understat_slice


class Command(BaseCommand):
    help = "Full refresh Understat player-season source rows for a competition-season slice."
    ...
    def handle(self, *args, **options) -> None:
        ...
        if run.status != IngestionRunStatus.SUCCESS:
            raise CommandError(run.error_detail or "Understat ingestion failed")
        self.stdout.write(self.style.SUCCESS(f"Understat run {run.id} succeeded ({run.stats})"))
