...
class EventProfileMaterializationTests(TestCase):
    ...
    def test_player_and_team_contract_is_hand_calculated(self):
        ...
        self.assertEqual(len(total.action_grid), 384); self.assertEqual(sum(c["raw_count"] for c in total.action_grid), 4)
        self.assertEqual(total.action_grid[0]["raw_count"], 1); self.assertEqual(total.action_grid[0]["share"], 1 / 4); self.assertEqual(total.action_grid[0]["per90_count"], .5)
        ...
        self.assertEqual((len(team.action_grid), len(team.opponent_action_grid), len(team.pass_flow)), (384, 384, 1))
        self.assertEqual(sum(c["raw_count"] for c in team.action_grid), 2); self.assertEqual(sum(c["raw_count"] for c in team.opponent_action_grid), 2)
        self.assertEqual(team.pass_flow[0]["column"], 0); self.assertEqual(team.pass_flow[0]["row"], 0)
        self.assertEqual(team.pass_flow[0]["completed_count"], 1); self.assertEqual(team.pass_flow[0]["share"], 1.0)
        self.assertEqual((team.pass_flow[0]["mean_origin_x"], team.pass_flow[0]["mean_origin_y"]), (0.0, 0.0))
        self.assertEqual((team.pass_flow[0]["mean_destination_x"], team.pass_flow[0]["mean_destination_y"]), (100.0, 100.0))
        self.assertAlmostEqual(team.pass_flow[0]["mean_length_metres"], 125.096, places=3)
    ...
    def test_public_gates_offline_and_command_scope(self):
        ...
        self.assertIsNotNone(result); self.assertEqual(succeeded.stats["formula_version"], "event_profiles_v3")
