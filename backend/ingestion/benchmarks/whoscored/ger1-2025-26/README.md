...
and match the semantic output digests in the Stage 7 report. The original role
digests in the baseline report included version-row timestamps and foreign keys,
so they changed on every equivalent rebuild. Stage 7 excludes that metadata and
also verifies the replacement rows field-by-field against their predecessors.
