...
import type { PlayerPassFilter, PlayerPassOutcome } from '../../types/eventMaps'
    ...
    <section aria-label="Player event maps" className="relative">
      ...
      <div className="grid items-start gap-3 lg:grid-cols-12">
        {profile.modules.passMap.available ? (
          <EventMapCard className="lg:col-span-8" expanded={expanded === 'passes'} onExpandedChange={next => setExpanded(next ? 'passes' : null)} title="Pass map" description={`${passQuery.data?.totalMatching.toLocaleString() ?? '—'} ${PASS_OUTCOMES.find(item => item.value === passOutcome)?.label.toLowerCase()} · ${PASS_FILTERS.find(item => item.value === passFilter)?.label.toLowerCase()} in this scope.`} controls={(
            ...
            <MapStage map="passes" expanded={expanded} setExpanded={setExpanded}>
              {passQuery.isLoading ? <EventMapNotice kind="loading" title="Loading pass rows" /> : passQuery.isError || !passQuery.data ? (
                ...
                <PortraitPitch passes={passQuery.data.passes} eventSelectionMode="click" selectedEventId={selection?.kind === 'pass' ? selection.id : null} onSelectedEventChange={setSelection} ariaLabel={`${profile.playerName} ${PASS_OUTCOMES.find(item => item.value === passOutcome)?.label.toLowerCase()} ${PASS_FILTERS.find(item => item.value === passFilter)?.label.toLowerCase()} pass map. Attacking left to right.`} />
        ...
        {profile.modules.shotMap.available ? (
          <EventMapCard className="lg:col-span-4" expanded={expanded === 'shots'} onExpandedChange={next => setExpanded(next ? 'shots' : null)} title="Shot map" description={profile.shots.length === 0 ? 'No shots were recorded in this scope.' : shotPitchView === 'attacking-half' ? 'Attacking half shown; all shots originate beyond halfway.' : 'Full pitch shown because this scope includes a shot from behind halfway.'} footer={(
            ...
            <MapStage map="shots" expanded={expanded} setExpanded={setExpanded}>
              {profile.shots.length ? <PortraitPitch shots={profile.shots} pitchView={shotPitchView} eventSelectionMode="click" selectedEventId={selection?.kind === 'shot' ? selection.id : null} onSelectedEventChange={setSelection} ariaLabel={`${profile.playerName} shot map. ${shotPitchView === 'attacking-half' ? 'Attacking half' : 'Full pitch'}; attacking left to right.`} /> : <EventMapNotice kind="empty" title="No shots recorded" />}
            </MapStage>
          </EventMapCard>
        ) : null}
