...
function InlineText({ content, fallback = '' }: { content: InlineContent; fallback?: string }) {
  ...
  return content.map((run, index) => {
    const text = <>{run.bold ? <strong>{run.italic ? <em>{run.text}</em> : run.text}</strong> : run.italic ? <em>{run.text}</em> : run.text}</>
    if (run.reference) return <a key={index} href={editorialEntityPath(run.reference)} className="inline-flex rounded-sm border border-electric/35 bg-electric-dim/35 px-1 py-0.5 text-electric hover:border-electric hover:bg-electric hover:text-mat">{text}</a>
    const url = run.link ? safeExternalUrl(run.link) : ''
    return url ? <a key={index} href={url} target="_blank" rel="noreferrer" className="border-b border-electric/60 text-electric hover:text-ink">{text}</a> : <span key={index}>{text}</span>
  })
}
